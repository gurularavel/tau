                ...
                <x-admin.crud.card :routeName="'internship_programs.update'" :method="'update'" :model="$model" :routeNameForBackButton="'internship_programs'"
                            ...
                            <x-admin.crud.tab-pane :key="$key">
                                ...
                                <x-admin.crud.card-body-row>
                                    ...
                                    <div class="project-items-wrapper" data-locale="{{ $locale }}">
                                        ...
                                        @forelse ($items as $index => $item)
                                            <div class="d-flex gap-2 mb-2 project-item-row">
                                                <input type="text"
                                                    name="internship_items[{{ $index }}][{{ $locale }}][title]"
                                                    class="form-control"
                                        ...
                                        @empty
                                            <div class="d-flex gap-2 mb-2 project-item-row">
                                                <input type="text"
                                                    name="internship_items[0][{{ $locale }}][title]"
                                    ...
                                    <div class="advantage-items-wrapper" data-locale="{{ $locale }}">
                                        @php
                                            $items = $model->advantageItems;
                                        @endphp

                                        @forelse ($items as $index => $item)
                                            <div class="d-flex gap-2 mb-2 advantage-item-row">
                                                <input type="text"
                                                    name="advantage_items[{{ $index }}][{{ $locale }}][title]"
                                                    class="form-control"
                                                    value="{{ $item->translate($locale)->title ?? '' }}"
                                                    placeholder="Title">
                                        ...
                                        @empty
                                            <div class="d-flex gap-2 mb-2 advantage-item-row">
                                                <input type="text"
                                                    name="advantage_items[0][{{ $locale }}][title]"
                                                    class="form-control" placeholder="Title">
                    ...
                    <div class="card">
                        <div class="card-body row">
                            ...
                            <div class="mb-3 col-lg-4">
                                <x-admin.crud.input :locale="''" :model="$model" :columnName="'place_count'"
                    ...
                    <x-admin.crud.image.card :title="$title">
                        <x-admin.crud.image.card-body>
                            <x-admin.crud.image.main-image :columnValue="$model->image" :name="'image'" :folderName="'internship_programs'" />
                            {{-- <x-admin.crud.image.other-images :model="$model" :routeName="'internship_programs'"
...
<script>
    let advanatageProjectItemIndex = {{ $model->advantageItems->count() ?: 1 }};
    ...
    document.addEventListener('click', function(e) {
        ...
        if (e.target.classList.contains('advantage-add-item')) {
            ...
            <div class="d-flex gap-2 mb-2 advantage-item-row">
                <input type="text"
                       name="advantage_items[${advanatageProjectItemIndex}][${locale}][title]"
                       class="form-control"
                       placeholder="Title">
                <button type="button" class="btn btn-danger advantage-remove-item">✕</button>
            </div>
