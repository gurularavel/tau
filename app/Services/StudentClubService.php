...
class StudentClubService extends BaseCrudService implements StudentClubServiceInterface
{
    use FileManagable;

    private const FOLDER = 'student_clubs';

    public function __construct(
        StudentClubRepositoryInterface $repository,
        )
    {
    ...
    public function create(array $payload, ?UploadedFile $file = null): Model
    {
        if (!is_null($file) && isset($payload['image'])) {
            ...
            $images = $this->multiUpload('student_club_images', $payload['images']);
