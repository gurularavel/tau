...
use App\Repositories\Contracts\InternshipProgramRepositoryInterface;
...
class InternshipProgramService extends BaseCrudService implements InternshipProgramServiceInterface
{
    use FileManagable;

    private const FOLDER = 'internship_programs';

    public function __construct(
        InternshipProgramRepositoryInterface $repository,
        )
    {
    ...
    public function create(array $payload, ?UploadedFile $file = null): Model
    {
        if (!is_null($file) && isset($payload['image'])) {
            ...
            $images = $this->multiUpload('internship_programs_images', $payload['images']);
