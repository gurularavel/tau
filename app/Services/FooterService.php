...
class FooterService extends BaseCrudService implements FooterServiceInterface
{
    use FileManagable;

    private const FOLDER = 'footers';

    public function __construct(
        FooterRepositoryInterface $repository,
        )
    {
    ...
    public function create(array $payload, ?UploadedFile $file = null): Model
    {
        if (!is_null($file) && isset($payload['image'])) {
            ...
            $images = $this->multiUpload('footers_images', $payload['images']);
