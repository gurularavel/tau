...
use App\Repositories\Contracts\ContainerItemRepositoryInterface;
...
class ContainerItemService extends BaseCrudService implements ContainerItemServiceInterface
{
    use FileManagable;

    private const FOLDER = 'container_items';

    public function __construct(ContainerItemRepositoryInterface $repository)
    {
        parent::__construct($repository);
    }
public function updateOrCreateWithTranslations($id, array $data, array $translations, $file = null)
{
    $item = $id ? \App\Models\ContainerItem::find($id) : new \App\Models\ContainerItem();
    ...
    if ($file && $file instanceof \Illuminate\Http\UploadedFile && $file->isValid()) {
        ...
        if ($item->image && file_exists(public_path('uploads/container_items/' . $item->image))) {
            @unlink(public_path('uploads/container_items/' . $item->image));
        }

        // Yeni şəkili yükləyirik
        $fileName = time() . '_' . rand(111, 999) . '.' . $file->getClientOriginalExtension();
        $file->move(public_path('uploads/container_items'), $fileName);
        $data['image'] = $fileName;
    } else {
