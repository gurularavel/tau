...
class StudentProjectService extends BaseCrudService implements StudentProjectServiceInterface
{
    use FileManagable;

    private const FOLDER = 'student_projects';

    public function __construct(
        StudentProjectRepositoryInterface $repository,
        )
    {
    ...
    public function create(array $payload, ?UploadedFile $file = null): Model
    {
        if (!is_null($file) && isset($payload['image'])) {
            ...
            $images = $this->multiUpload('student_project_images', $payload['images']);
